...
`lattice-algo-path-lab` is centered on algorithms work in OCaml. The technical target is: Package an OCaml local lab for path analysis with append-only fixtures, checkpoint recovery checks, and documented operating limits.
...
The score is deterministic. The current threshold is 167. Risk has a penalty of 7, latency has a penalty of 3, and weight contributes a bonus of 4.
